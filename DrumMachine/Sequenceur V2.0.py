...
kick = pygame.image.load("Images/Seq-Kick.png").convert_alpha()
snare = pygame.image.load("Images/Seq-Snare.png").convert_alpha()
clap = pygame.image.load("Images/Seq-Clap.png").convert_alpha()
hats = pygame.image.load("Images/Seq-Hats.png").convert_alpha()
silence = pygame.image.load("Images/Seq-Silence.png").convert_alpha()
objet = pygame.image.load("Images/FondPapier.png").convert_alpha()
boite = pygame.image.load("Images/Fond.png").convert_alpha()
...
kick_position = screen.blit(kick,(posx[0],posy[0]))
snare_position = screen.blit(snare, (posx[1], posy[1]))
clap_position = screen.blit(clap, (posx[2], posy[2]))
hats_position = screen.blit(hats, (posx[3], posy[3]))
silence_position = screen.blit(silence, (posx[4], posy[4]))

#Position des boutons des instruments lorsque la souris est pressée
...
while Boucle:
    for event in pygame.event.get():
        ...
        screen.blit(objet, (0,0))  
        screen.blit(boite,(100, 70))
        screen.blit(kick, (poscasex_kick, poscasey_kick))
        screen.blit(snare, (poscasex_snare, poscasey_snare))
        screen.blit(clap, (poscasex_clap, poscasey_clap))
        screen.blit(hats, (poscasex_hats, poscasey_hats))
        screen.blit(silence, (poscasex_silence, poscasey_silence))
        pygame.display.flip()
...
# boucle déplacement d'image pour le KICK
        if event.type == pygame.MOUSEBUTTONDOWN and event.button == 1 and kick_position.collidepoint(event.pos):
            ...
            BoutonRelacher(posx[0],posy[0], kick)
            selected_kick = 0
            Detection_Sequenceur(kickson, kick, posx[0], posy[0])

        elif event.type == MOUSEMOTION and selected_kick == 1:
            SourisMouvement(kick, kick_position)
...
# boucle déplacement d'image pour le SNARE
        if event.type == pygame.MOUSEBUTTONDOWN and event.button == 1 and snare_position.collidepoint(event.pos):
            ...
            BoutonRelacher(posx[1],posy[1], snare)
            selected_snare = 0
            Detection_Sequenceur(snareson, snare, posx[1], posy[1])
                
        elif event.type == MOUSEMOTION and selected_snare == 1:
            SourisMouvement(snare, snare_position)
...
# boucle déplacement d'image pour le CLAP
        if event.type == pygame.MOUSEBUTTONDOWN and event.button == 1 and clap_position.collidepoint(event.pos):
            ...
            BoutonRelacher(posx[2],posy[2], clap)
            selected_clap = 0
            Detection_Sequenceur(clapson, clap, posx[2], posy[2])
                
        elif event.type == MOUSEMOTION and selected_clap == 1:
            SourisMouvement(clap, clap_position)
...
# boucle déplacement d'image pour les HATS
        if event.type == pygame.MOUSEBUTTONDOWN and event.button == 1 and hats_position.collidepoint(event.pos):
            ...
            BoutonRelacher(posx[3],posy[3], hats)
            selected_hats = 0
            Detection_Sequenceur(hatsson, hats, posx[3], posy[3])
                
        elif event.type == MOUSEMOTION and selected_hats == 1:
            SourisMouvement(hats, hats_position)
...
# boucle déplacement d'image pour le SILENCE
        if event.type == pygame.MOUSEBUTTONDOWN and event.button == 1 and silence_position.collidepoint(event.pos):
            ...
            BoutonRelacher(posx[4],posy[4], silence)
            selected_silence = 0
            Detection_Sequenceur(silenceson, silence, posx[4], posy[4])
                
        elif event.type == MOUSEMOTION and selected_silence == 1:
            SourisMouvement(silence, silence_position)
